...
# Глобальный dict для хранения временных данных покупок
purchase_data = {}
...
def main_menu_kb() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🎁 Бесплатная подписка")],
...
def subscription_type_kb() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📡 Обычный доступ (100₽/мес)", callback_data="sub_normal")],
            [InlineKeyboardButton(text="🛡️ Полный доступ (100₽/мес)", callback_data="sub_antiblock")],
            [InlineKeyboardButton(text="🚀 Оба варианта (100₽/мес)", callback_data="sub_both")],
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(
                text=f"📅 1 месяц ({format_currency(durations[1])})",
                callback_data="dur_1m"
            )],
            [InlineKeyboardButton(
                text=f"📅 3 месяца ({format_currency(durations[3])})",
                callback_data="dur_3m"
            )],
            [InlineKeyboardButton(
                text=f"📅 6 месяцев ({format_currency(durations[6])})",
                callback_data="dur_6m"
            )],
            [InlineKeyboardButton(
                text=f"📅 1 год ({format_currency(durations[12])})",
                callback_data="dur_12m"
            )],
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    db = SessionLocal()
    try:
        # Проверяем, не реферальная ли это ссылка
        ...
        # Если есть реферал и пользователь новый (только что создан)
        if referrer_id and referrer_id != message.from_user.id:
            # Проверяем, что пользователь действительно новый
    ...
    welcome_text = """
👋 Добро пожаловать в VPN Subscription Bot!

Здесь вы можете:
• 💳 Купить высокоскоростную VPN подписку
...
@router.message(ClientStates.main_menu)
async def handle_main_menu(message: Message, state: FSMContext):
    ...
    elif text == "🎁 Бесплатная подписка":
        await handle_free_trial(message, state)
    ...
    elif text == "ℹ️ Поддержка":
        await message.answer(
            "ℹ️ Поддержка\n\n"
            "❓ Возникли вопросы? Напишите нам!\n\n"
            "📧 Email: support@maximikvpn.fun\n"
            "💬 Telegram: @maximikvpn\n\n"
...
async def handle_free_trial(message: Message, state: FSMContext):
    ...
    try:
        ...
        try:
            ...
            # Если не удалось проверить, даем тест в любом случае
            is_subscribed = True
        ...
        if success:
            ...
            # Проверяем, нужно ли выдать реферальный бонус
            if user.referrer_id:
                # Проверяем, есть ли запись в таблице рефералов
                from shared.models import Referral
                referral = db.query(Referral).filter(
                    Referral.referred_id == str(message.from_user.id),
                ...
                if referral:
                    ...
                    try:
                        await message.bot.send_message(
                            ...
                            f"💰 Вам начислено {format_currency(REFERRAL_REWARD)}!"
                        )
                    except Exception as e:
...
async def check_subscription(callback: CallbackQuery, state: FSMContext):
    ...
    if is_subscribed:
        # Удаляем старое сообщение и запускаем выдачу теста
...
async def select_duration(callback: CallbackQuery, state: FSMContext):
    ...
    purchase_data[callback.from_user.id].update({
        "days": days,
        "price": price
    })
    ...
    sub_type_labels = {
        ...
        "antiblock": "🛡️ Полный доступ (обход)",
...
async def enter_custom_days(message: Message, state: FSMContext):
    ...
    try:
        ...
        purchase_data[message.from_user.id].update({
            "days": days,
            "price": price
        })
...
async def confirm_purchase(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Списываем деньги и добавляем подписку
        user.balance -= price
        success, msg = add_days_to_subscription(
            user,
            purchase["days"],
            purchase["sub_type"],
            db
        ...
        if success:
            await callback.message.edit_text(
                f"✅ Покупка успешна!\n\n"
                f"📊 Тип: {purchase['sub_type'].upper()}\n"
                f"📅 На период: {purchase['days']} дней\n"
                f"💰 Списано: {format_currency(price)}\n"
...
async def confirm_payment(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(
            ...
            "Администратор подтвердит платеж и пополнит ваш баланс в течение 15 минут.",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="🔙 В меню", callback_data="back_to_menu")]
                ]
...
async def download_config(callback: CallbackQuery):
    """Скачать конфиг подписки как файл"""
...
async def extend_subscription(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "⏳ Продление подписки\n\n"
        "Выберите тип доступа для продления:",
        reply_markup=subscription_type_kb()
    )
    await state.set_state(ClientStates.select_sub_type)
